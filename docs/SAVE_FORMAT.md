...
阶段 10 引入行囊（`V3ToV4` 补空表）：

```json
{
  "schemaVersion": 4,
  "inventory": { "gift_jiu": 2 }
}
```

- `inventory`：物品 id → 件数。市集购入、送礼消耗；件数归零移除键，
  校验器拒绝 0 件条目与目录外物品 id。

## v3 增量（历史）

阶段 5–10 引入案件与生涯字段（`V2ToV3` 补空结构）：

- `cases`：案件进度（状态/开卷与限期戳/线索/推论/指认/结语键/冤案旗标），
  查了一半的案子线索一条不能丢。
- `kaokeGrades`：历年考课九等（迁转与贬谪判定的依据）。
- `housing`：现居宅邸 id（默认 `hut`）。
- `codex`：已解锁典籍词条 id。
- `marriage`：议亲对象 / 六礼进度 / 已婚。
...
- 现存迁移：`V0ToV1`（早期开发格式：money_guan 浮点贯 → moneyWen 整数文，
  拼音属性键改名，补 offices/ledger/flags 空结构）、`V1ToV2`（通缉值 + NPC 状态）、
  `V2ToV3`（案件/考课/宅邸/典籍/婚姻）、`V3ToV4`（行囊）。
